...
function updateCore(t, dt){
  ...
  coreGroup.rotation.y = -.18 + Math.sin(t * .07) * .030;
  coreGroup.rotation.x = .18 + Math.sin(t * .10) * .016;
  coreGroup.rotation.z = -.06 + Math.sin(t * .05) * .006;

  arcGroup.rotation.y = Math.sin(t * .05) * .012;
  arcGroup.rotation.z = Math.cos(t * .03) * .010;
  dustCloud.rotation.y += dt * .0035;
  dustCloud.rotation.z -= dt * .0016;

  nodeMat.size = .044 + neuralTension * .010 + burstEnergy * .003;
  nodeMat.opacity = .90 + neuralTension * .06;
  haloMat.opacity = .07 + neuralTension * .06;
  edgeMat.opacity = .125 + neuralTension * .075 + burstEnergy * .018;
  warmMat.opacity = .105 + neuralTension * .085 + burstEnergy * .020;
  hotMat.opacity = .60 + neuralTension * .20 + Math.sin(t*1.5) * .025;
  dustMat.opacity = .10 + neuralTension * .04;
  fissureLine.material.opacity = .12 + neuralTension * .08;

  keyLight.intensity = 4.7 + neuralTension * 2.8 + burstEnergy * .8;
  warmLight.intensity = 4.0 + neuralTension * 2.4 + burstEnergy * .8;
  rimLight.intensity = 2.7 + neuralTension * 1.5;

  camera.position.x = Math.sin(t * .045) * .030;
  camera.position.y = -.04 + Math.cos(t * .07) * .020;
  camera.position.z = 7.4 + Math.sin(t * .035) * .035;
  camera.lookAt(0,.04,.18);
